...
//多态
int
ccn_parse_PublisherID2(struct ccn_buf_decoder *d, struct ccn_parsed_ant *pi)
{
    int res = -1;
    int iskey = 0;
    unsigned pubstart = d->decoder.token_index;
                   ...
                   struct ccn_indexbuf *components)
{
    ...
    int res;
  //  int i;
    if (ccn_buf_match_dtag(d, CCN_DTAG_Interest)|| ccn_buf_match_dtag(d, CCN_DTAG_Ant)) {///////////////
...
////////////////////////////////////////By   ice////////////////////////////////////////////////////////////////////
int
ccn_parse_ant(const unsigned char *msg, size_t size,
                   struct ccn_parsed_ant *ant,
                   struct ccn_indexbuf *components)
{
    struct ccn_buf_decoder decoder;
    struct ccn_buf_decoder *d = ccn_buf_decoder_start(&decoder, msg, size);
    ...
    int res;
    int i;
    if (ccn_buf_match_dtag(d, CCN_DTAG_Ant)) {     
	 if (components == NULL) {//null
            /* We need to have the component offsets. */
            components = ccn_indexbuf_create();
		...
		printf("s1 =  \n");////////////////////////////////////////////////////////
            ...
            if (components == NULL) return(-1);
            res = ccn_parse_ant(msg, size, ant, components);
            ccn_indexbuf_destroy(&components);
            return(res);
        }
        ccn_buf_advance(d);
        ant->offset[CCN_PA_B_Name] = d->decoder.element_index;
        ant->offset[CCN_PA_B_Component0] = d->decoder.index;
        ncomp = ccn_parse_Name(d, components);//返回前缀由几部分组成
/*
        //Start: Added by Shen Li
        printf("((((((((((((( In ccn_parse_ant ncomp = %d, %d, components: ", ncomp, components->n); 

        //End: Added by Shen Li
*/
        if (d->decoder.state < 0) {
		printf("st2 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
            memset(ant->offset, 0, sizeof(ant->offset));
            return(d->decoder.state);
        }
        ant->offset[CCN_PA_E_ComponentLast] = d->decoder.token_index - 1;
        ant->offset[CCN_PA_E_Name] = d->decoder.token_index;
        ant->prefix_comps = ncomp;
        ant->offset[CCN_PA_B_LastPrefixComponent] = components->buf[(ncomp > 0) ? (ncomp - 1) : 0];
        ant->offset[CCN_PA_E_LastPrefixComponent] = components->buf[ncomp];
        /* optional MinSuffixComponents, MaxSuffixComponents */
        ant->min_suffix_comps = 0;
        ant->max_suffix_comps = 32767;
        ant->offset[CCN_PA_B_MinSuffixComponents] = d->decoder.token_index;
        res = ccn_parse_optional_tagged_nonNegativeInteger(d,
                                                           CCN_DTAG_MinSuffixComponents);
        ant->offset[CCN_PA_E_MinSuffixComponents] = d->decoder.token_index;
printf("st3 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        if (res >= 0)
            ant->min_suffix_comps = res;
        ant->offset[CCN_PA_B_MaxSuffixComponents] = d->decoder.token_index;
        res = ccn_parse_optional_tagged_nonNegativeInteger(d,
                                                           CCN_DTAG_MaxSuffixComponents);
        ant->offset[CCN_PA_E_MaxSuffixComponents] = d->decoder.token_index;
printf("st4 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        if (res >= 0)
            ant->max_suffix_comps = res;
        if (ant->max_suffix_comps < ant->min_suffix_comps)
            return (d->decoder.state = -__LINE__);
        /* optional PublisherID */
        res = ccn_parse_PublisherID2(d, ant);////////////////////////////////////////////////
        /* optional Exclude element */
        ant->offset[CCN_PA_B_Exclude] = d->decoder.token_index;
        res = ccn_parse_Exclude(d);
        ant->offset[CCN_PA_E_Exclude] = d->decoder.token_index;
printf("st5 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        /* optional ChildSelector */
        ant->offset[CCN_PA_B_ChildSelector] = d->decoder.token_index;
        res = ccn_parse_optional_tagged_nonNegativeInteger(d,
                         CCN_DTAG_ChildSelector);
printf("st6 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        if (res < 0)
            res = 0;
        ant->orderpref = res;
        ant->offset[CCN_PA_E_ChildSelector] = d->decoder.token_index;
        if (ant->orderpref > 5)
            return (d->decoder.state = -__LINE__);        
printf("st7 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        /* optional AnswerOriginKind */
        ant->offset[CCN_PA_B_AnswerOriginKind] = d->decoder.token_index;
        ant->answerfrom = ccn_parse_optional_tagged_nonNegativeInteger(d,
                         CCN_DTAG_AnswerOriginKind);
        ant->offset[CCN_PA_E_AnswerOriginKind] = d->decoder.token_index;
printf("st8 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        if (ant->answerfrom == -1)
            ant->answerfrom = CCN_AOK_DEFAULT;
        /* optional Scope */
        ant->offset[CCN_PA_B_Scope] = d->decoder.token_index;
        ant->scope = ccn_parse_optional_tagged_nonNegativeInteger(d,
                         CCN_DTAG_Scope);
        ant->offset[CCN_PA_E_Scope] = d->decoder.token_index;
printf("st9 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        if (ant->scope > 9)
                return (d->decoder.state = -__LINE__);
        if ((ant->answerfrom & CCN_AOK_EXPIRE) != 0 &&
            ant->scope != 0)
                return (d->decoder.state = -__LINE__);
printf("st10 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
    //Start:By ICE:
	/*Time*/
	ant->offset[CCN_PA_B_Time] = d->decoder.token_index;
	ant->time =7;
        ant->time = ccn_parse_optional_tagged_nonNegativeInteger(d,
                         CCN_DTAG_Time);////////////////////////////////////////ice
        ant->offset[CCN_PA_E_Time] = d->decoder.token_index;
    //End:By ICE:
printf("st11 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        /* optional InterestLifetime */
        ant->offset[CCN_PA_B_InterestLifetime] = d->decoder.token_index;
        res = ccn_parse_optional_tagged_BLOB(d, CCN_DTAG_InterestLifetime, 1, 8);
        if (res >= 0)
            magic |= 20100401;
        ant->offset[CCN_PA_E_InterestLifetime] = d->decoder.token_index;

 	/// printf("((((((((((((( = %d,  ", d->decoder.state); ////////////////////////////////////////////
printf("st12 =%d \n",d->decoder.state);////////////////////////////////////////////////////////

        /* optional Nonce */
        ant->offset[CCN_PA_B_Nonce] = d->decoder.token_index;
        res = ccn_parse_optional_tagged_BLOB(d, CCN_DTAG_Nonce, 4, 64);
        ant->offset[CCN_PA_E_Nonce] = d->decoder.token_index;
printf("st13 =%d \n",d->decoder.state);////////////////////////////////////////////////////////

        ant->offset[CCN_PA_B_OTHER] = d->decoder.token_index;
        /* this is for local use */
        ccn_parse_optional_tagged_nonNegativeInteger(d, CCN_DTAG_FaceID);
        ant->offset[CCN_PA_E_OTHER] = d->decoder.token_index;
printf("st14 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
        ccn_buf_check_close(d);
        ant->offset[CCN_PA_E] = d->decoder.index;
printf("st15 =%d \n",d->decoder.state);////////////////////////////////////////////////////////
    }
    else
        return (d->decoder.state = -__LINE__);
