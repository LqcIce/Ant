...
////////////////////////////////////////By Ice//////////////////////////////////////////////////////////////////////
static void
ccn_construct_ant(struct ccn *h,
                       struct ccn_charbuf *name_prefix,
                       struct ccn_charbuf *interest_template,
                       struct expressed_interest *dest)
{
    ...
    dest->lifetime_us = CCN_INTEREST_LIFETIME_MICROSEC;		//4*1000000

    c->length = 0;
    ccnb_element_begin(c, CCN_DTAG_Ant);
    ccn_charbuf_append(c, name_prefix->buf, name_prefix->length);
    res = 0;
    if (interest_template != NULL) {
        struct ccn_parsed_ant pa = { 0 };//解析模板
        res = ccn_parse_ant(interest_template->buf,
                                 interest_template->length, &pa, NULL);
	printf("((((((((((((( = %d,  ", res);
        if (res >= 0) {
            intmax_t lifetime = ccn_ant_lifetime(interest_template->buf, &pa);
            // XXX - for now, don't try to handle lifetimes over 30 seconds.
            if (lifetime < 1 || lifetime > (30 << 12))
                NOTE_ERR(h, EINVAL);
            else
                dest->lifetime_us = (lifetime * 1000000) >> 12;
            start = pa.offset[CCN_PA_E_Name];
            size = pa.offset[CCN_PA_B_Nonce] - start;
            ccn_charbuf_append(c, interest_template->buf + start, size);//添加name

            start = pa.offset[CCN_PA_B_OTHER];
            size = pa.offset[CCN_PA_E_OTHER] - start;
            if (size != 0)
                ccn_charbuf_append(c, interest_template->buf + start, size);
        }
        else
                     ...
                     struct ccn_charbuf *interest_template)
{
    ...
    interest->magic = 0x7059e5f4;
    ccn_construct_ant(h, namebuf, interest_template, interest);
    if (interest->interest_msg == NULL) {
        free(interest);
        hashtb_end(e);
        return(-1);
...
//	ccnd_msg(h, "==== %d",interest->interest_msg );/////////////////////////////
        ...
        int flags)
{
    ...
    md->closure.intdata = 1; /* tell upcall to re-express if needed */
    md->closure.refcount = 1;
    res = ccn_express_ant(h, name, &md->closure, interest_template);
    if (res >= 0)
        res = ccn_run(h, timeout_ms);
    if (res >= 0)
        res = md->res;
