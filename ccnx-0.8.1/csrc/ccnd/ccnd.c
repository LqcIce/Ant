...
///////////////////////////////////////////////////////////////////////////////////
//		res=pa->time;
//	ccnd_msg(h, "time %d", res);
	res=pi->offset[CCN_PI_B_Scope];
	ccnd_msg(h, "CCN_PI_B_Scope %d", res);
	res=pi->offset[CCN_PI_E_Scope];
	ccnd_msg(h, "CCN_PI_E_Scope %d", res);

	res=pi->offset[CCN_PI_B_InterestLifetime];
	ccnd_msg(h, "CCN_PI_B_Scope %d", res);
	res=pi->offset[CCN_PI_E_InterestLifetime];
	ccnd_msg(h, "CCN_PI_E_Scope %d", res);

	res=pi->offset[CCN_PI_B_Name];
	ccnd_msg(h, "CCN_PI_B_Scope %d", res);
	res=pi->offset[CCN_PI_E_Name];
	ccnd_msg(h, "CCN_PI_E_Scope %d", res);
////////////////////////////////////////////////////////////////////////////////////////////
...
int construct_ping_response(struct ccn *h, struct ccn_charbuf *data, 
        const unsigned char *interest_msg, const struct ccn_parsed_ant *pa, int expire)
{
    struct ccn_charbuf *name = ccn_charbuf_create();
    struct ccn_signing_params sp = CCN_SIGNING_PARAMS_INIT;
    int res;

    ccn_charbuf_append(name, interest_msg + pa->offset[CCN_PA_B_Name],
            pa->offset[CCN_PA_E_Name] - pa->offset[CCN_PA_B_Name]);
    ...
    res = ccn_sign_content(h, data, name, &sp, "ant time", strlen("ant time"));
...
//////////////////////////////////////////By  Ice/////////////////////////////////////////////////////////////////
static void
process_incoming_ant(struct ccnd_handle *h, struct face *face,
                          unsigned char *msg, size_t size)
{
    struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    struct ccn_parsed_ant parsed_ant = {0};
    struct ccn_parsed_ant *pa = &parsed_ant;
    int res;

    struct ccn_indexbuf *comps = indexbuf_obtain(h);

        res = ccn_parse_ant(msg, size, pa, comps);
    if (res < 0) {
        ccnd_msg(h, "error parsing Interest - code %d", res);
...
/*
	res=pi->min_suffix_comps;
	ccnd_msg(h, "min_suffix_comps %d", res);
	res=pi->max_suffix_comps;
	ccnd_msg(h, "max_suffix_comps %d", res);
*/
	res=pa->time;
	ccnd_msg(h, "time %d", res);
	res=pa->magic;
	ccnd_msg(h, "magic %d", res);
	res=pa->min_suffix_comps;
	ccnd_msg(h, "min_suffix_comps %d", res);
	res=pa->max_suffix_comps;
	ccnd_msg(h, "max_suffix_comps %d", res);
	res=pa->answerfrom;
	ccnd_msg(h, "answerfrom %d", res);
	res=pa->scope;
	ccnd_msg(h, "scope %d", res);


	res=pa->offset[CCN_PA_E_Name];
	ccnd_msg(h, "E_Name %d", res);
	res=pa->offset[CCN_PI_B_MinSuffixComponents];
	ccnd_msg(h, "CCN_PI_B_MinSuffixComponents, %d", res);
	res=pa->offset[CCN_PI_E_PublisherID];
	ccnd_msg(h, "CCN_PI_E_PublisherID %d", res);

        // Construct Data content with given Interest name.
        struct ccn_charbuf *data = ccn_charbuf_create();
        construct_ping_response(h, data, msg,pa, 4000);
//////////////////////////////  h    ,  p    ,  length
        res = ccn_put(h, data->buf, data->length);
        ccn_charbuf_destroy(&data);
	    //	fprintf(stderr, "Ant Pefect.\n");
}
                      ...
                      unsigned char *msg, size_t size, int pdu_ok)
{
    ...
    switch (dtag) {
	    ...
	    //fprintf(stderr, "Pefect Interest.\n");
  	    ccnd_msg(h, "============== Interest");
            return;
        case CCN_DTAG_ContentObject:
            process_incoming_content(h, face, msg, size);
	    //fprintf(stderr, "Pefect content.\n");
	    ccnd_msg(h, "==============  content");
            return;
        case CCN_DTAG_SequenceNumber:
        ...
        case CCN_DTAG_Ant:
	    ccnd_msg(h, "============== Ant");
	    process_incoming_ant(h, face, msg, size);
