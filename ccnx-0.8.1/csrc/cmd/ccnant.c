...
/////////////////////////////////////////////////////////////////////////////////////
        ...
        ccnb_element_begin(templ, CCN_DTAG_Ant);
		...
		time= GetCurrentTime();/////////////////////////////////////////获取时间
		ccnb_tagged_putf(templ, CCN_DTAG_Time, "%d", time);//添加时间
		...
		/* 	ccnb_element_begin(templ, CCN_DTAG_Time);
			ccnb_append_number(templ,
							   CCN_AOK_DEFAULT | CCN_AOK_STALE);
			ccnb_element_end(templ); </Time> */

  
	//ccnb_append_tagged_blob(templ, CCN_DTAG_Time, "11", sizeof("11"));
	//ccnb_element_begin(templ, CCN_DTAG_Time);
	//ccnb_append_number(templ,11);

	//ccnb_element_end(templ); /* </Time> */

	//ccnb_element_begin(templ, CCN_DTAG_Ant);    		/*MY work  ice*/
	//ccnb_element_end(templ); /* </Ant> */
	//ccnb_tagged_putf(templ, CCN_DTAG_Ant, "%d", ant);
  	//ccnb_tagged_putf(templ, CCN_DTAG_MinSuffixComponents, "%d", 110);
	//ccnb_tagged_putf(templ, CCN_DTAG_MaxSuffixComponents, "%d", 120);
        ccnb_element_end(templ); /* </ant> */
...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    //res = ccn_get(h, name, templ, timeout_ms, resultbuf, &pcobuf, NULL, get_flags);
      res = ccn_get_ant(h, name, templ, timeout_ms, resultbuf, &pcobuf, NULL, get_flags);
    if (res >= 0) {
        ptr = resultbuf->buf;
